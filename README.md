...
### 过滤流程
```
LLM回复 → 临时修改文本 → TTS读取过滤后内容 → 延迟恢复原始文本
           ↓                    ↓                    ↓
    应用所有过滤规则:        TTS合成语音        用户看到原始内容
    1. 颜文字移除 (正则匹配)
    2. 直接过滤词汇移除
    3. 词汇替换 (网络用语转换)
    4. 特殊符号移除 (表情符号等)
...
### 工作机制
- **临时修改**: 临时将文本修改为过滤后的版本供TTS使用
- **延迟恢复**: 0.1秒后自动恢复原始文本，确保用户看到原始内容
- **可靠恢复**: 使用 `call_later` 确保恢复时机准确
- **错误处理**: 恢复失败时记录警告日志，不影响功能
...
- **v0.3** (当前版本)
  - 🚀 **重大改进**: 简化过滤机制，使用临时修改+延迟恢复方案
  - 🔧 **架构优化**: 临时修改文本供TTS使用，延迟恢复确保用户看到原始内容
  - ⚙️ **配置增强**: 添加最大处理长度配置项，提高灵活性
  - 📊 **代码规范**: 修复导入语句位置，符合PEP 8规范
  - 🛡️ **稳定性提升**: 使用可靠的恢复机制，避免时机问题
  - 🔄 **简洁高效**: 代码更简洁，功能更可靠
